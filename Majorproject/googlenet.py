...
#def reading(self):
 #   s = open('imagenet_labels.txt', 'r').read()
  #  self.whip = eval(s)
   # return self.whip
...
if __name__ == "__main__":
    ...
    csv_logger = CSVLogger('Googlenet.csv')
    early_stopper = EarlyStopping(min_delta=0.001,patience=30)
    model_checkpoint = ModelCheckpoint('Googlenet.hdf5',monitor = 'val_loss', verbose = 1,save_best_only=True)
    ...
    #model.fit(train_images,[train_labels,train_labels,train_labels],batch_size=16,epochs=30,validation_split=0.1,callbacks=[lr_reducer, early_stopper,csv_logger,model_checkpoint])
    
    img = imresize(imread('rose.jpg', mode='RGB'), (224, 224)).astype(np.float32)
    img[:, :, 0] -= 123.68
    img[:, :, 1] -= 116.779
    img[:, :, 2] -= 103.939
    img[:,:,[0,1,2]] = img[:,:,[2,1,0]]
...
#for key,value in labels_list.iteritems():
    ...
    final_labels = {}
    final_text = open("finallabels.txt","r")
    lines = final_text.read().split('\n')
    lines = lines[0:len(lines)-1]
    for line in lines:
        string = re.split(r'\s+',line)
        #print(string)
        final_labels[string[0]]=string[1]
 ...
 999: 'toilet tissue, toilet paper, bathroom tissue'}
    ...
    print imagenet_labels[label]
"""    count = 0
    for root, dirs, files in os.walk("val/images_new"):
        path = root.split(os.sep)
        #print((len(path) - 1) * '---', os.path.basename(root)) 
        for file in files:
            img = imresize(imread("val/images_new/"+file, mode='RGB'), (224, 224)).astype(np.float32)
            img[:, :, 0] -= 123.68
            img[:, :, 1] -= 116.779
            img[:, :, 2] -= 103.939
            img[:,:,[0,1,2]] = img[:,:,[2,1,0]]
            ...
            #print(labels_list[file])
            actual_label = final_labels[labels_list[file]] 
            #print("Actual")
            #print(actual_label)
            #print("predicted")
